...
$.fn.Validform = function () {
    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            switch (checkexpession) {
                ...
                case "Amount":
	                {
	                	if (!isAmount(value, $(this).attr("eqvalue"))) {
	                		Validatemsg = errormsg + "必须为金额(最小精确到分)！\n";
	                		Validateflag = false;
	                		ValidationMessage($(this), Validatemsg); return false;
	                	}
	                	break;
                ...
                case "AmountOrNull":
	                {
	                	if (!isAmountOrNull(value, $(this).attr("eqvalue"))) {
	                		Validatemsg = errormsg + "必须为金额(最小精确到分)！\n";
	                		Validateflag = false;
	                		ValidationMessage($(this), Validatemsg); return false;
	                	}
                ...
                case "Code":
                {
                	if (!isCode(value, $(this).attr("eqvalue"))) {
                		Validatemsg = errormsg + "必须为'字母'|'数字'|'下划线'组成的字符！\n";
                		Validateflag = false;
                		ValidationMessage($(this), Validatemsg); return false;
                	}
                	break;
                ...
                case "CodeOrNull":
                {
                	if (!isCodeOrNull(value, $(this).attr("eqvalue"))) {
                		Validatemsg = errormsg + "必须为字母|数字|下划线组成的字符！\n";
                		Validateflag = false;
                		ValidationMessage($(this), Validatemsg); return false;
                	}
                	break;
    ...
    //判断输入的字符是否为金额或者null,空
    function isAmountOrNull(obj) {
    	var controlObj = $.trim(obj);
    	if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
    		return true;
    	}
    ...
    //判断输入的字符是否为包括下划线的任何单词字符或者null,空
    function isCodeOrNull(obj) {
    	var controlObj = $.trim(obj);
    	if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
    		return true;
    	}
